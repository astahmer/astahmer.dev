---
title: '"Why would I choose Panda CSS ?"'
publishedAt: 2023-12-16
description: "And why it's easier to answer this question"
slug: 'why-would-i-choose-panda-css'
isPublish: true
atUri: 'at://did:plc:rn2ur6nqbw5hddbbw2khckhw/site.standard.document/3mjmncyu7ui2o'
---
...
import DebugUtilityExample from '@/components/examples/debug-utility.astro'
...
This time, I feel like this question is more about what **strengths** Panda has to offer. And I've got tons of answers
for that !
...
<details>
<summary>1. [An overview of the main features](#build-time-css)</summary>

- 1.0. [Build-time CSS](#build-time-css)
- 1.1. [Atomic CSS](#atomic-css)
- 1.2. [Type-safety](#built-in-type-safety)
- 1.3. [Batteries included](#batteries-included)
- 1.4. [Extendable](#extendable)
- 1.5. [CSS-in-JS](#css-in-js)
- 1.6. [Style props](#style-props)
- 1.7. [Merging styles](#merging-styles)
- 1.8. [Static analysis](#static-analysis)
- 1.9. [Runtime dynamic styling](#runtime-dynamic-styling)
- 1.10. [Static generator](#static-generator)
- 1.11. [Modern](#modern)
- 1.12. [Multi-Theme tokens](#multi-theme-tokens)
- 1.13. [Virtual color palette](#virtual-color-palette)

</details>
<details>
<summary>2. [A look at the possible use-cases](#use-cases)</summary>

- 2.0. [Styling solution](#styling-solution)
- 2.1. [Design system](#design-system--component-library)
- 2.2 [Token generator](#token-generator)
- 2.3 [Styling engine](#styling-engine)

</details>
<details>
<summary>3. [A summary of the DX](#developer-experience-dx)</summary>

- 3.0. [Easy to learn](#easy-to-learn)
- 3.1. [Easy to use](#easy-to-use)
- 3.2. [Easy to customize](#easy-to-customize)
- 3.3. [Easy to distribute](#easy-to-distribute)

</details>
<details>
<summary>4. [A concise API reference with examples](#api-examples)</summary>

- 4.0. [css](#css)
- 4.1. [cx](#cx)
- 4.2. [cva](#cva)
- 4.3. [config recipes](#recipes)
- 4.4. [slot recipes](#slot-recipes)
- 4.5. [sva](#sva)
- 4.6. [styled](#styled)
- 4.7. [patterns](#patterns--jsx-patterns)
- 4.8. [utility](#utility)
- 4.9. [Runtime token()](#runtime-token)
- 4.10. [Compile-time string token()](#compile-time-string-token)
- 4.11. [JSX style props](#jsx-style-props)
- 4.12. [JSX css prop](#jsx-css-prop)
- 4.13. [.raw(xxx) merging](#rawxxx-merging)
- 4.14. [Runtime conditions](#runtime-conditions)
- 4.15. [CSS nesting](#css-nesting)
- 4.16. [colorPalette](#colorpalette)
- 5.0. [Theme](#theme)
- 5.1. [tokens](#tokens)
- 5.2. [semantic tokens](#semantic-tokens)
- 5.3. [conditions](#conditions)
- 5.4. [breakpoints](#breakpoints)
- 5.5. [shorthands](#shorthands)
- 5.6. [textStyles](#textstyles)
- 5.7. [keyframes](#keyframes)
- 6.0. [CLI](#cli)
- 7.0. [Online playground](#online-playground)
...
that we can generate a raw CSS file that you can use (and cache !) **anywhere**.
...
This also means that you don't need a VSCode extension to get autocompletion and type-safety, it's **built-in**. _(tho,
we've been working on a VSCode extension that provides **super-powered autocompletion and even more features**)_
...
ℹ️ Note: By default, [`config.strictTokens`](https://panda-css.com/docs/references/config#stricttokens) is set to
...
- `@pandacss/preset-panda`, which contains the [default theme](https://panda-css.com/docs/customization/theme)
  _([`keyframes`](https://panda-css.com/docs/customization/theme#keyframes) /
  [`breakpoints`](https://panda-css.com/docs/concepts/responsive-design) /
  [`tokens`](https://panda-css.com/docs/theming/tokens) /
  [`textStyles`](https://panda-css.com/docs/theming/text-styles))_
...
with your [theme](https://panda-css.com/docs/customization/theme)
_([`keyframes`](https://panda-css.com/docs/customization/theme#keyframes) /
[`breakpoints`](https://panda-css.com/docs/concepts/responsive-design) /
[`tokens`](https://panda-css.com/docs/theming/tokens) / [`textStyles`](https://panda-css.com/docs/theming/text-styles))
/ [conditions](https://panda-css.com/docs/concepts/conditional-styles) /
[utilities](https://panda-css.com/docs/utilities/background) / [patterns](https://panda-css.com/docs/concepts/patterns)_
to fit your needs.
...
- Stricter [type-safety](#built-in-type-safety) ? Set
...
- Too many properties suggested in the
  ...
  the [`css`](#jsx-css-prop) special property) or `none` to disable them completely.
...
[type-safety](#built-in-type-safety) all over the place.

And that works with most JSX-like frameworks: [React](https://panda-css.com/docs/installation/nextjs) /
...
This `raw` function will return what you passed in (except when used on `patterns`, it will transform the object back to
a `css`-compatible object), and **serves as a marker for the `static analysis`**, to tell the Panda `extractor` that
you're creating a `style object` and we should generate the CSS for it.

```ts del={4-5} ins={7-8} "css.raw" "blue.500" "bg.500"
// file-1.ts
import { css } from 'styled-system/css'
...
```tsx "isSelected ? 'blue.500' : 'yellow.500'"
import { css } from '../styled-system/css'
...
const Button = (props) => {
  ...
  return (
    <button
      className={css({
        color: getColor('foreground'), // ✅ this will statically evaluate to "yellow.100"
      })}
    >
      {props.children}
    </button>
...
// ✅ the final CSS would successfully include :
// `color: blue.500`, `color: yellow.500`
```
...
With Panda CSS, you could use an `atomic recipe` (2nd argument of the `styled factory` or `cva`) to keep the style
definition static.

Not only does it achieve the same result but you also unlock the possibilities of having multiple styles bound to the
same variant, and of course you can create as many variants as you want.
...
ℹ️ Note that this specific `data-selected` example is
...
That being said, if you need to support older browsers you can always use
...
Since Panda is based on CSS variables, that also means you can override the `data-theme` anytime in the DOM, and it will
automatically update the colors. You could even replace the theme colors by overriding the current CSS variables with a
`style={xxx}` attribute, useful for example if you want to let your users customize the colors at runtime.
...
<small>ℹ️ You can hover the buttons for even more madness !</small>
...
ℹ️ Note that you could duplicate that `button` object in the `semanticTokens` and just change the color values to get
...
It's just the CSS you know already, using `camelCase` instead of `kebab-case`, with the additions of `type-safety`
autosuggestions for `tokens` and `conditions` while still allowing raw values and selectors.
...
Since all Panda does it generating a CSS, you can use it anywhere with our [`CLI`](#cli). ¹

We also provide a `postcss` plugin to skip the CSS file import step and having another process (CLI with `--watch`) to
run in parallel.

While Panda brings many features to the table, you can choose to use a very small API surface, using only the `css`
function to style your components, and that's it. If you're not a fan of [`CSS-in-JS`](#css-in-js), that's also fine.

Maybe colocation isn't your thing at all ? It's fine, you can keep your style objects in a separate file. It _just
works_.

[Getting started](https://panda-css.com/docs/installation/postcss) is as easy as installing the CLI and running the
`panda init -p` command in your project.
...
Even though Panda CSS comes with [batteries-included](#batteries-included), you can customize it to fit your needs.

Using the `panda.config` you can extend the default one or create your own theme (`tokens` / `breakpoints` /
`textStyles` / etc .. ) and even create your own `utilities` / `conditions` / `patterns.`

You can disable/omit features that you don't need, like `shorthands`, `jsxStyleProps` or even `jsxFramework` entirely !
If you feel like `type-safety` must be enforced, you can set `strictTokens` to `true`.
...
Using a preset is the easiest way to distribute your Design System (`tokens`, `breakpoints`, `recipes`, etc) with other
Panda users.
...
You can also distribute your library CSS by generating a static file using the [CLI](#cli), and use it anywhere.
...
> ⚠️ This of course means that you shouldn't directly expose any Panda API (`css` / `recipes` / etc..) to your users
> because it would not generate any additional CSS.
...
- make a package dedicated to the `config.outdir` (defaults to `styled-system`, for this example we'll use
  `@acme/styled-system`)
- both you and your users **must** set your [`config.importMap`](https://panda-css.com/docs/references/config#importmap)
...
1. Panda uses the [`importMap`](https://panda-css.com/docs/references/config#importmap) to know which imports should
   matching your theme ([`tokens`](#tokens) / [`recipes`](#recipes) / etc..)
2. by sharing the same `module specifier` (the right part of imports, e.g `abc` in `import xxx from 'abc'`), your
   bundler (`vite` for example) will be able to deduplicate the lightweight JS runtime (`config.outdir`) between the one
   from your library and the one from your users.

If you don't share the same module specifier, users will end up bundling the code for 2 different `css` functions, `cva`
etc; one from your library (`@acme/styled-system`) and one from your users outdir (`styled-system` by default) which is
not ideal.
...
Let's take a little break. If you reached this far, you probably have a good idea of what `Panda CSS` is, and what it
can do for you.

The part below is mostly concise explanations/examples of what the documentation already covers (I shamelessly took
small parts of it), so that you can have a quick overview/reference of the API surface.

This should also help searching for a specific feature, since the documentation is pretty long, and this is a one-page
reference searchable with `cmd+f`.
...
In the static analysis step we extract each arguments, resolving each style object property/value combinations, and
generating an atomic CSS rule for each of them.

At runtime, it transforms the object-syntax (that you provide) to a class string composed of atomic classes. Most Panda
features are using the `css` function under the hood. (`cva`, `recipes`, `patterns`, `styled` factory, etc...). That
means that anything you can do with the `css` function, you can do the same in those features.
...
- Has a [`.raw()`](#raw-marker) function to mark the object as a style object for the Panda static extractor
...
⚠️ You should NOT rename JSX properties, like using `myColor={color}` instead of `color={color}`. This is due to
...
You can also create a `styled` component [using another one as basis](https://play.panda-css.com/JUa2RxiZle) (1st arg),
and the previous `atomic recipe` base and variants styles will automatically be deep merged with the new one. (new wins
in case of conflict)
...
export default defineConfig({
  ...
  jsxFactory: 'panda', // ℹ️ you can rename the `jsx` factory to `panda` if you want (defaults to `styled`)
})
...
const App = () => (
  // ✅ only the `css` property is available when using `jsxStyleProps: "minimal"`
...
<TabItem label="recursive-merging.ts">

```tsx
// https://play.panda-css.com/JUa2RxiZle
...
const Merged = styled(StyledButton, sizes)

const App = () => {
  return (
    <Merged visual="solid" size="lg">
      With size merged
    </Merged>
  )
}
...
This is a built-in [utility](#utility), included in the base preset, that you can use to debug your styles, it will add
an `outline` attribute to the element and all its children. Simple, but quite useful !
...
If you wonder how it works, here's the full code of the `debug` utility:
...
> ⚠️ We recommend not using descendant selectors as they can lead to specificity issues when managing style overrides.
...
Tokens are at the core of the Panda CSS design system. They're a way to define your design system values in a
centralized place, and then reference them in your styles.

> Design tokens in Panda are largely influenced by [the W3C Token Format](https://tr.designtokens.org/format/)

They are grouped in [predefined categories](https://panda-css.com/docs/theming/tokens#token-types), the exhaustive list
is: [zIndex](https://panda-css.com/docs/theming/tokens#z-index),
[opacity](https://panda-css.com/docs/theming/tokens#opacity),
[colors](https://panda-css.com/docs/theming/tokens#colors), [fonts](https://panda-css.com/docs/theming/tokens#fonts),
[fontSizes](https://panda-css.com/docs/theming/tokens#font-sizes),
[fontWeights](https://panda-css.com/docs/theming/tokens#font-weights),
[lineHeights](https://panda-css.com/docs/theming/tokens#line-heights),
[letterSpacings](https://panda-css.com/docs/theming/tokens#letter-spacings),
[sizes](https://panda-css.com/docs/theming/tokens#sizes), [shadows](https://panda-css.com/docs/theming/tokens#shadows),
[spacing](https://panda-css.com/docs/theming/tokens#spacings), [radii](https://panda-css.com/docs/theming/tokens#radii),
[borders](https://panda-css.com/docs/theming/tokens#borders),
[durations](https://panda-css.com/docs/theming/tokens#durations),
[easings](https://panda-css.com/docs/theming/tokens#easings),
[animations](https://panda-css.com/docs/theming/tokens#animations),
[gradients](https://panda-css.com/docs/theming/tokens#gradients),
[assets](https://panda-css.com/docs/theming/tokens#assets)

- Each token will create a CSS variable
- You can nest tokens indefinitely
- [Nested tokens](https://panda-css.com/docs/theming/tokens#token-nesting) can be used to create groups and will
  generate dot-separated names
...
Same as [tokens](#tokens) but can reference other tokens.
...
const className = css({
  ...
  // targeting children is possible (⚠️ but NOT recommended)
  '& span': {
    bg: 'blue.500',
  },
...
<Tabs>
  <TabItem label="object-syntax.ts">
...
```
  ...
  <TabItem label="array-syntax.ts">

```tsx {6}
// Panda also accepts arrays as values for responsive styles.
...
```
  ...
  <TabItem label="breakpoints-targets.ts">
...
```
  ...
  <TabItem label="hide-from-below.ts">
...
`Shorthands` are a way to create shorter names for properties that you use often. They are enabled by default in the
config and the built-in `presets` come with a few of them.
...
[Panda CLI](https://panda-css.com/docs/installation/cli), included when you install `@pandacss/dev`, allows you to
generate CSS in any project that has a `panda.config.{js,ts,etc}` file.

Although the postcss plugin is very convenient, it's not required to use Panda CSS. You can use the CLI to generate the
CSS and runtime, and then use them in your project however you want.

CLI reference is [here](https://panda-css.com/docs/references/cli).

Here's a quicky summary:

- [`panda init`](https://panda-css.com/docs/references/cli#panda-init) will help you get started with a
  `panda.config.ts`, or can also be used if you don't remember the `@layer` order

- [`panda`](https://panda-css.com/docs/references/cli#panda) will generate the CSS (from your `config.include`) + and
  lightweight runtime in your `config.outdir`, and watch for changes with the `-w` or `--watch` flag. It's the same as
  `panda cssgen && panda codegen`

- [`panda cssgen`](https://panda-css.com/docs/references/cli#panda-cssgen) will only generate the CSS (from your
  `config.include`)

- `panda cssgen [artifact]`, will only generate the CSS related to the given `artifact`, which can be one of:
  `preflight` | `tokens` | `static` | `global` | `keyframes`

- `panda cssgen --minimal` will only generate the CSS related to the style usage found in your `config.include`

- [`panda codegen`](https://panda-css.com/docs/references/cli#panda-codegen) will generate the lightweight runtime in
  your `config.outdir`

- [`panda debug`](https://panda-css.com/docs/references/cli#panda-debug) will generate 1 `.css` file and 1 `.json` file
  for each of your source files, allowing you to see what was generated (css) and what was extracted (json) by file.

- [`panda ship`](https://panda-css.com/docs/references/cli#panda-ship) will generate 1 `.json` file that contains the
  summary of the style usage found from your source files. You can then use this `.json` file in any `config.include` as
  if you had included the source files, which can be useful to distribute the CSS of a npm package using Panda without
  publishing the source files.

ℹ️ If you're concerned about the performance of the postcss plugin, the CLI is a great alternative. We don't have much
control over how often the postcss plugin will be called, but we can control how many times we call the CLI. (e.g only
when we need to)
...
I hope this post will help give more exposure to Panda CSS and its many features. I'm really excited to see what people
will build with it and how it will evolve in the future.
